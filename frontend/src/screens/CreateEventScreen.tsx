    ...
    try {
      ...
      try {
        ...
        if (apiError.response) {
          ...
          Alert.alert('错误', `创建事件失败: ${apiError.response.status} - ${JSON.stringify(apiError.response.data)}`);
          ...
          Alert.alert('错误', `创建事件失败: ${apiError.message}`);
        }
      }
    } catch (error) {
    ...
    <TouchableWithoutFeedback onPress={() => {
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          ...
          <Text style={styles.title}>创建新事件</Text>
